package net.mcreator.elliemoreand;
...
public class ElliemoreANDVariables {
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "anukedays_mapvars";
		public boolean Radioactive_downfall = false;
		public double Radioactive_downfall_Time = 0;
		public double Radioactive_downfall_start = 0;
		public MapVariables() {
			super(DATA_NAME);
		}
		...
		public void readFromNBT(NBTTagCompound nbt) {
			Radioactive_downfall = nbt.getBoolean("Radioactive_downfall");
			Radioactive_downfall_Time = nbt.getDouble("Radioactive_downfall_Time");
			Radioactive_downfall_start = nbt.getDouble("Radioactive_downfall_start");
		}

		@Override
		public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
			nbt.setBoolean("Radioactive_downfall", Radioactive_downfall);
			nbt.setDouble("Radioactive_downfall_Time", Radioactive_downfall_Time);
			nbt.setDouble("Radioactive_downfall_start", Radioactive_downfall_start);
			return nbt;
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				ElliemoreAND.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				ElliemoreAND.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "anukedays_worldvars";
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				ElliemoreAND.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				ElliemoreAND.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage> {
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world) {
			if (context.side == Side.SERVER) {
				...
				if (message.type == 0)
					ElliemoreAND.PACKET_HANDLER.sendToAll(message);
				else
					ElliemoreAND.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
